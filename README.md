...
| 用户类型 | 使用场景 |
|----------|----------|
| 普通用户 | 注册登录、关注板块、浏览信息流、弹窗发帖、评论互动 |
| 吧主 | 编辑自己管理的板块、发布板块公告、删除板块（非系统板块） |
| 管理员 | 后台管理用户 / 板块 / 帖子 / 评论 / 全站公告、审核板块申请 |
...
- **用户系统**：邮箱注册与验证、登录/登出、个人资料（头像 / 昵称 / 简介）
- **板块系统**：板块列表、按板块浏览帖子
- **帖子系统**：富文本图文帖（最多 9 张图）、列表 / 详情、作者编辑 / 软删除
...
- **布局优化**：内容区加宽（`--page-max-width: 1600px`）、统一三栏 CSS
- **统一侧边栏**：`UnifiedSidebar`（主页 / 个人中心 + 关注 / 推荐 / 全部板块）
- **双层公告**：全站公告（首页右栏）+ 板块公告（板块页 / 帖子详情右栏）
- **帖子详情三栏**：左导航 + 中正文 + 右板块公告
- **公告预览**：标题 + 4 行内容预览 + 相对更新时间
- **板块删除**：吧主 / 管理员可在编辑弹窗软删除板块（系统板块除外）
- **图片圆角**：帖子图片统一 8px 圆角
...
│   └── phase3/
│       └── phase3-requirements.md     # 三期需求与任务拆解
│
...
└── forum-web/                         # 前端 Vue 3 + Vite
    ...
    │   ├── components/                # layout / post / announcement / board 等
    │   ├── stores/                    # Pinia（user / app / boardFollow / postEditor）
    │   ├── api/                       # Axios 接口封装
    │   ├── router/
    │   └── styles/global.css          # 全局样式 + 三栏布局变量
    └── package.json
```
...
| 产品需求 (PRD) | [`context/PRD-社区论坛系统.md`](context/PRD-社区论坛系统.md) | 完整功能需求 |
...
| 6 | 顶栏搜索关键词 | 跳转 `/search`，可切换板块/帖子/用户 Tab |
| 7 | 用户下拉 →「我的板块申请」 | 可申请新板块（需满足注册天数与发帖数） |
